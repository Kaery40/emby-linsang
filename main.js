...
function adjustStyles() {
    ...
    document.querySelectorAll('.misty-banner-cover').forEach((el) => {
        el.style.maxHeight = isMobile ? '40vh' : '60vh';
    });

    document.querySelectorAll('.misty-banner-info').forEach((el) => {
        el.style.fontSize = isMobile ? '0.9rem' : '1.2rem';
    });

    document.querySelectorAll('.misty-banner-info button').forEach((el) => {
        if (isMobile) {
    ...
    document.querySelectorAll('.misty-banner-logo').forEach((el) => {
        el.style.height = isMobile ? '2rem' : '4rem';
    });

    if (isMobile) {
        document.querySelectorAll('.misty-banner-info, .misty-banner-logo').forEach((el) => {
            el.style.display = 'none';
        });
        document.querySelector('.misty-banner-body').style.opacity = '0';
    } else {
        document.querySelectorAll('.misty-banner-info, .misty-banner-logo').forEach((el) => {
            el.style.display = '';
        });
        document.querySelector('.misty-banner-body').style.opacity = '1';
    }
}
